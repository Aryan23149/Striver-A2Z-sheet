...
int solveFrogJumpOptimized(int n,vector<int> &heights){
    int next1=0;
    int next2=0;
    int current;
    for(int i=n-2;i>=0;i--){
...
int frogJump(int n, vector<int> &heights)
{
    ...
    // tc: O(n) SC:O(n+n+n)
    // vector<int> dp(n,-1);
    // return solveFrogJumpMemo(0,n,heights,dp);
    ...
    // dp[n]=0;
    // return solveFrogJumpTabu(n+1,heights,dp);

    // using optimized space and tabulation
    // tc: O(N) and sc(O(1))
    return solveFrogJumpOptimized(n,heights);
}